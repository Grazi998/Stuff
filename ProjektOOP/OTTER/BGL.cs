...
namespace OTTER
{
    ...
    public partial class BGL : Form
    {
        ...
        private void syncRate_Click(object sender, EventArgs e)
        {
        ...
        bool prepreka=false;
        int brojac = 0;
        //int[] LevelNiz = { 5, 6, 7, 8, 9, 12, 16, 24, 25, 35 };
        ...
        private delegate void MenuChooseHandler();
        private event MenuChooseHandler _menuChoose;

        private delegate void LoadLevelHandler();
        private event LoadLevelHandler _loadLevel;

        private void SetupGame()
        {
            //1. setup stage
            razina = new ChooseLevel();
            _menuChoose += PokreniMenu;
            _menuChoose.Invoke();

            lista = ChooseLevel.LevelNiz;
            this.Width = ChooseLevel.Width;
            this.Height = ChooseLevel.Height;

            _loadLevel += UcitajLevel;
            _loadLevel.Invoke();
            SetStageTitle("Roller");
            setBackgroundPicture("backgrounds\\white.jpg");
            //none, tile, stretch, center, zoom
            setPictureLayout("stretch");            
            
            
            //UČITAJ IZBORNIK IGRE
            

                      
            //2. add sprites
            ball = new Kugla("sprites\\ball.png", 0, 0);            
            ball.RotationStyle = "AllAround";
            Game.AddSprite(ball);


            //3. scripts that start
            
            //MessageBox.Show("Za pokreniti igru pritisni slovo \"Z\" te pricekaj par sekundi dok se učita (~10sec)");
            Game.StartScript(Move);
        ...
        private int Move()
        {
            while (START)
            {
                ISPIS = "\nWxH: ;" + this.Width.ToString() + "x" + this.Height.ToString() + "\nBrojač: " + brojac.ToString();

                if (sensing.KeyPressed(Keys.A))
                {
                    ball.Pokrenuta = true;
                    sensing.KeyUp();
                    while (ball.Pokrenuta)
                    {
                        ball.X -= GameOptions.Speed;
                        Wait(0.01);
                    }
                    
                }

                else if (sensing.KeyPressed(Keys.D))
                {
                    ball.Pokrenuta = true;
                    sensing.KeyUp();
                    while (ball.Pokrenuta)
                ...
                //        //ISPIS = "X=" + ball.X.ToString() + "\t Y=" + ball.Y.ToString() + "\nPotez: " + moves.ToString();

                //        if (ball.Y+ball.Heigth>= GameOptions.DownEdge || sensing.KeyPressed("P"))
                //        {
                //            moves -= 1;
                //            sensing.KeyUp();
                //            break;                            
                ...
                //        ball.Y += GameOptions.Speed;
                //        foreach (Sprite s in polja)
                //        {
                //            if (s.TouchingSprite(ball))
                //            {
                //                if (s.CostumeName == "prepreka")
                //                {
                //                    prepreka = true;
                //                    moves -= 1;
                //                    ball.X = previous.X;
                ...
                //        if (ball.Y <= 0 || sensing.KeyPressed("P"))
                //        {
                //            moves -= 1;
                //            sensing.KeyUp();
                //            break;                            
                ...
                //        if (ball.X + ball.Width >= GameOptions.RightEdge || sensing.KeyPressed("P"))
                //        {
                //            moves -= 1;
                //            sensing.KeyUp();
                //            break; ;
                ...
                //    {                        
                //        //ISPIS = "X=" + ball.X.ToString() + "\t Y=" + ball.Y.ToString() + "\nPotez: " + moves.ToString();
                //        if (ball.X <= 0 || sensing.KeyPressed("P"))
                //        {
                //            moves -= 1;
                //            sensing.KeyUp();
                //            break;
                ...
                //UČITAJ LEVEL
                //else if (sensing.KeyPressed(Keys.Z))
                //{
                //    sensing.KeyUp();
                //    using(LoadingForm frm=new LoadingForm(UcitajLevel))
                //    {
                //        frm.ShowDialog();
                //    }                
        ...
        void UcitajLevel()
        {
            List<Polja> ListaPolja = new List<Polja>();
            for (int j = 0; j < GameOptions.RightEdge; j += 100)
            {
                for (int i = 0; i < GameOptions.DownEdge; i += 100)
                {
                    
                    Polja polje = new Polja(i, j);

                    if (j == 0 || j == GameOptions.DownEdge - 100 || i == 0 || i == GameOptions.RightEdge - 100)
                    {
                        polje.CurrentCostume = new Bitmap("sprites\\prepreka.png");
                        polje.CostumeName = "prepreka";
            ...
            //{
            //    ListaPolja[i].CurrentCostume = new Bitmap("sprites\\prepreka.png");
            //    ListaPolja[i].CostumeName = "prepreka";
            //    ListaPolja[i].SetVisible(true);
            //}

            foreach (Sprite s in ListaPolja)
            {
                Game.AddSprite(s);
                if (s.CostumeName != "prepreka")
                {
