...
class AdminView():
    ...
    def a_ucenici(ucenici=[]):
        return  template('administracija_html/Ucenici_adminA', title="Popis ucenika", ucenici=ucenici)
    @staticmethod
    def dodavanje_ucenika():
        return template('administracija_html/dodavanje_ucenika')
    @staticmethod
    ...
    def nastavnik_popis(nastavnici=[]):
        return template('administracija_html/popis_nastavnikaA', title='Popis nastavnici',nastavnici=nastavnici)

    @staticmethod
    def nastavnik_dodaj():
        return template('administracija_html/add_nastavnikA', title='Dodavanje nastavnika')

    @staticmethod
    def dodavanje_nastavnik_validacija(val):
        return template('administracija_html/add_nastavnik_validA', title='Validacija nastavnika',validacija=val)

    @staticmethod
    def edit_nastavnik(mb,ime,prezime):
        return template('administracija_html/edit_nastavnikA', title='Uredi nastavnika',mb=mb,ime=ime,prezime=prezime)

    @staticmethod
    def valid_edit_nastavnik(mb, ime, prezime, validacija):
        return template('administracija_html/edit_nastavnik_validA', title='Uredi nastavnika validacija', mb=mb, ime=ime, prezime=prezime,validacija=validacija)

    @staticmethod
    def nastavnik_delete(mb,ime,prezime):
        return template('administracija_html/nastavnik_deleteA',title='Brisanje nastavnika', mb=mb, ime=ime, prezime=prezime)

    @staticmethod
    def kordinacija(pn=[], nastavnici=[],pp=[]):
        return  template('administracija_html/koordinacijaA',title='Predmet-nastavnik',pn=pn,nastavnici=nastavnici,pp=pp)
